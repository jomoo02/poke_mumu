...
import type { ConditionItem, ConditionOtherCase } from '../types/condition';

function sortEnCondition(conditions: ConditionItem[]) {
  const generalConditions: ConditionItem[] = [];
  const lastConditions: ConditionItem[] = [];
...
function sortKoCondition(conditions: ConditionItem[]) {
  const firstConditions: ConditionItem[] = [];
  const lastConditions: ConditionItem[] = [];
  const generalConditions: ConditionItem[] = [];
...
export function sortConditionByLanguage(conditions: ConditionItem[], language: Language) {
  if (language === 'en') {
...
export function getAreaInfoWithKey(key: AreaKey) {
  const {
    mossyRock,
    icyRock,
    magneticField,
...
export function getOtherConditionContent(poke: ConditionOtherCase): LanguageContentType {
  const defaultContent = {
    en: '',
    ko: '',
  };

  const otherCasePokeLocaleContent: Record<ConditionOtherCase, LanguageContentType> = {
    sirfetchD: {
